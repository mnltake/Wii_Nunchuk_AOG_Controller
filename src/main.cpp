...
#define I2C_SDA  0 // Nuncuck SDA
#define I2C_CLK  26 // Nuncuck CLK
...
void setup() {
    Serial.begin(9600);
    M5.begin();
    Wire.begin(I2C_SDA,I2C_CLK,400000);
...
void loop() {  
    ...
    if (isBleConnected && M5.BtnA.wasPressed()) {
    ...
    if (isBleConnected && M5.BtnB.wasPressed()) {
        // button has been pressed: type message
        Serial.println("TAB");
        M5.Lcd.fillScreen(BLACK);
        M5.Lcd.printf("\n TAB");
    ...
    if (nunchuk_read() ){
      if( isBleConnected && nunchuk_buttonZ()==1) {
        Serial.print("Z=");
        Serial.println(nunchuk_buttonZ(), DEC);
        M5.Lcd.fillScreen(BLACK);
        M5.Lcd.printf("AUTO Steer\n\n On/Off");
        typeText("a");
        delay(250);
      }
      if( isBleConnected && nunchuk_buttonC()==1) {
        Serial.print("C=");     
        Serial.println(nunchuk_buttonC(), DEC);
        M5.Lcd.fillScreen(BLACK);
        M5.Lcd.printf("Manual\n\n Section");
        typeText("m");
        delay(250);
      }
      if( isBleConnected && nunchuk_joystickX() < -90 ) {
         Serial.print("X=");     
         Serial.println(nunchuk_joystickX() , DEC);
         M5.Lcd.fillScreen(BLACK);
         M5.Lcd.printf("Move line\n\n LEFT");
        typeText("t");
        delay(250);
      }
      if( isBleConnected && nunchuk_joystickX() > 90 ) {
      ...
      if( isBleConnected && nunchuk_joystickY() < -90 ) {
         Serial.print("Y=");    
         Serial.println(nunchuk_joystickY() , DEC);
         M5.Lcd.fillScreen(BLACK);
         M5.Lcd.printf("Flag \n\n Point");
        typeText("g\t\t\t\n");
        delay(250);
